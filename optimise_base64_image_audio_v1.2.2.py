...
__version__     = "1.2.2"
__comments__    = "03/18/26 EW: 1.2.2v - Fixed CSS background-image processing for small images that were being corrupted with double url() wrapper; 01/14/26 EW: 1.2.1v - Added support to skip processing of SVG images (data:image/svg+xml;base64); 01/14/26 EW: 1.2.0v - Added progress bars and configurable verbose mode; 10/20/24 EW: 1.1v - Added loader"
...
def optimize_image(image_data, mime_type, quality=80, convert_to_webp=True, max_dimension=None, verbose=False):
    ...
    if len(image_data) < 1024:  # 1KB
        log_verbose(f"Image file too small to optimize: {len(image_data)} bytes")
    ...
    if not is_valid_image(image_data, mime_type):
        log_verbose(f"Warning: Invalid or unsupported image format for {mime_type}")
    ...
    try:
        log_verbose(f"Processing image file: {mime_type}, size: {len(image_data)} bytes, quality: {quality}")
        ...
        if max_dimension and (img.width > max_dimension or img.height > max_dimension):
            ...
            log_verbose(f"Resized image from {img.width}x{img.height} to {new_width}x{new_height}")
        ...
        if convert_to_webp:
            ...
            if len(webp_data) < len(image_data):
                ...
                log_verbose(f"WebP conversion results:")
                log_verbose(f"  Original size: {original_size:,} bytes")
                log_verbose(f"  Optimized size: {new_size:,} bytes")
                log_verbose(f"  Savings: {savings:,} bytes ({savings_percent:.2f}%)")
        ...
        if len(optimized_data) < len(image_data):
            ...
            log_verbose(f"Image optimization results:")
            log_verbose(f"  Original size: {original_size:,} bytes")
            log_verbose(f"  Optimized size: {new_size:,} bytes")
            log_verbose(f"  Savings: {savings:,} bytes ({savings_percent:.2f}%)")
            ...
            log_verbose("Optimized image not smaller than original, keeping original")
...
def optimize_svg(svg_data, verbose=False):
    ...
    try:
        log_verbose(f"Processing SVG file, size: {len(svg_data)} bytes")
        ...
        if len(compressed_data) < len(svg_data):
            ...
            log_verbose(f"SVG gzip compression results:")
            log_verbose(f"  Original size: {original_size:,} bytes")
            log_verbose(f"  Optimized size: {new_size:,} bytes")
            log_verbose(f"  Savings: {savings:,} bytes ({savings_percent:.2f}%)")
        ...
        if len(minified_data) < len(svg_data):
            ...
            log_verbose(f"SVG minification results:")
            log_verbose(f"  Original size: {original_size:,} bytes")
            log_verbose(f"  Optimized size: {new_size:,} bytes")
            log_verbose(f"  Savings: {savings:,} bytes ({savings_percent:.2f}%)")
        ...
        log_verbose("SVG optimization did not reduce size, keeping original")
        image_detail['status'] = 'skipped_no_reduction'
        image_detail['reason'] = "No size reduction achieved"
        image_stats['details'].append(image_detail)
        image_stats['skipped_no_reduction'] += 1
...
def optimize_audio(audio_data, mime_type, bitrate=128, verbose=False):
    ...
    try:
        ...
        log_verbose(f"FFmpeg version check result: {result.returncode}")
        log_verbose(f"FFmpeg stdout: {result.stdout.decode('utf-8', errors='ignore')[:100]}...")
        log_verbose(f"FFmpeg stderr: {result.stderr.decode('utf-8', errors='ignore')[:100]}...")
    ...
    if len(audio_data) < 10240:  # 10KB
        log_verbose(f"Audio file too small to optimize: {len(audio_data)} bytes")
    ...
    try:
        log_verbose(f"Processing audio file: {mime_type}, size: {len(audio_data)} bytes, target bitrate: {bitrate}kbps")
        ...
        log_verbose(f"Created temp files: input={input_path}, output={output_path}")
        ...
        log_verbose(f"Running FFmpeg command: {' '.join(cmd)}")
        
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
        
        log_verbose(f"FFmpeg result: {result.returncode}")
        log_verbose(f"FFmpeg stderr: {result.stderr.decode('utf-8', errors='ignore')}")
        ...
        if result.returncode == 0:
            ...
            log_verbose(f"Audio optimization results:")
            log_verbose(f"  Original size: {original_size:,} bytes")
            log_verbose(f"  Optimized size: {new_size:,} bytes")
            log_verbose(f"  Savings: {savings:,} bytes ({savings_percent:.2f}%)")
            ...
            if len(optimized_data) < len(audio_data):
                ...
                log_verbose("Optimized audio not smaller than original, keeping original")
            ...
            log_verbose(f"FFmpeg error: {result.stderr.decode('utf-8', errors='ignore')}")
        ...
        try:
            ...
            log_verbose(f"Error cleaning up temp files: {e}")
...
def process_css_background_image(match, options=None):
    """
    Process a CSS background-image data URI match and return optimized version.
    This function is specifically for CSS background-image processing where the match
    includes the url() wrapper. It extracts just the data URI, processes it, and
    returns just the data URI (without url() wrapper) so the caller can wrap it.
    
    Args:
        match: Regex match object where group(1) is the data URI
        options: Processing options dictionary
        
    Returns:
        str: The data URI (optimized or original), without url() wrapper
    """
    if options is None:
        options = {}
    
    # Get the data URI from the capture group (without url() wrapper)
    # The regex captures the data URI in group(1)
    try:
        data_uri = match.group(1)
    except IndexError:
        # Fallback to group(0) if no capture group
        data_uri = match.group(0)
    
    # If the data_uri still has url() wrapper, extract just the data part
    if data_uri.startswith('url('):
        # Extract the data URI from url(...)
        inner_match = re.search(r'url\(["\']?(data:[^;]+;base64,[^"\')\s]+)["\']?\)', data_uri)
        if inner_match:
            data_uri = inner_match.group(1)
    ...
    if not is_valid_base64(base64_data):
        log_verbose(f"Warning: Invalid base64 data found for {mime_type}")
        return data_uri
    
    # Skip SVG images entirely - don't process them at all
    if mime_type == 'image/svg+xml':
        return data_uri
    ...
    try:
        ...
        if len(binary_data) < options.get('min_size', 1024):
            log_verbose(f"CSS background image too small to optimize: {len(binary_data)} bytes")
            return data_uri
        ...
        log_verbose(f"Error processing CSS background image: {e}")
        return data_uri
...
def optimize_html_file(input_path, output_path, options=None, inflate_js_content=None):
    ...
    try:
        ...
        # Count total data URIs for progress tracking
        total_data_uris = len(re.findall(r'data:[^;]+;base64,[^"\'\s]+', html_content))
        current_progress = [0]  # Use list to allow modification in nested function
        
        def process_with_progress(match, tag_name=None):
            """Wrapper to show progress while processing"""
            result = process_data_uri(match, tag_name, options)
            current_progress[0] += 1
            if not VERBOSE and total_data_uris > 0:
                print_progress_bar(current_progress[0], total_data_uris,
                                 prefix='Optimising:',
                                 suffix=f'{current_progress[0]}/{total_data_uris} items',
                                 length=40)
            return result
        
        def process_css_bg_with_progress(match):
            """Wrapper to show progress while processing CSS background images"""
            result = process_css_background_image(match, options)
            current_progress[0] += 1
            if not VERBOSE and total_data_uris > 0:
                print_progress_bar(current_progress[0], total_data_uris,
                                 prefix='Optimising:',
        ...
        html_content = re.sub(img_pattern, lambda m: re.sub(r'src=["\'](data:[^;]+;base64,[^"\']+)["\']',
                                                           lambda n: process_with_progress(n, 'img'), m.group(0)), html_content)
        ...
        html_content = re.sub(source_srcset_pattern, lambda m: re.sub(r'srcset=["\'](data:[^;]+;base64,[^"\']+)["\']',
                                                                 lambda n: process_with_progress(n, 'source'), m.group(0)), html_content)
        ...
        html_content = re.sub(source_src_pattern, lambda m: re.sub(r'src=["\'](data:audio/[^;]+;base64,[^"\']+)["\']',
                                                               lambda n: f'src="{process_with_progress(n, "source")}"', m.group(0)), html_content)
        ...
        html_content = re.sub(audio_pattern, lambda m: re.sub(r'src=["\'](data:audio/[^;]+;base64,[^"\']+)["\']',
                                                           lambda n: f'src="{process_with_progress(n, "audio")}"', m.group(0)), html_content)
        
        # Process CSS background images
        # Use process_css_bg_with_progress which correctly handles small images
        css_pattern = r'background-image:\s*url\(["\']?(data:[^;]+;base64,[^"\')\s]+)["\']?\)'
        html_content = re.sub(css_pattern, lambda m: re.sub(r'url\(["\']?(data:[^;]+;base64,[^"\')\s]+)["\']?\)',
                                                          lambda n: f'url({process_css_bg_with_progress(n)})', m.group(0)), html_content)
        ...
        # After processing HTML content (e.g., after optimising images/audio)
...
def process_chunk(chunk, options=None):
    ...
    # Process CSS background images
    # Use process_css_background_image which correctly handles small images
    # by returning just the data URI (without url() wrapper) so we can wrap it here
    css_pattern = r'background-image:\s*url\(["\']?(data:[^;]+;base64,[^"\')\s]+)["\']?\)'
    chunk = re.sub(css_pattern, lambda m: re.sub(r'url\(["\']?(data:[^;]+;base64,[^"\')\s]+)["\']?\)',
                                                lambda n: f'url({process_css_background_image(n, options)})', m.group(0)), chunk)
...
def main():
    ...
    parser.add_argument('-v', '--verbose', action='store_true',
                        help='Enable verbose mode with detailed processing information')
    ...
    log_verbose("Optimization options:")
    log_verbose(f"  - Image quality: {args.image_quality}")
    log_verbose(f"  - Audio bitrate: {args.audio_bitrate}kbps")
    log_verbose(f"  - Convert to WebP: {args.webp}")
    if args.max_dimension:
        log_verbose(f"  - Max dimension: {args.max_dimension}px")
    log_verbose(f"  - Use Base85 encoding: {args.base85}")
    log_verbose(f"  - Minimum size to optimize: {args.min_size} bytes")
    log_verbose(f"  - Minimum compression ratio: {args.min_ratio:.2f}")
    log_verbose(f"  - Process in chunks: {args.chunks}")
    log_verbose(f"  - Verbose mode: {args.verbose}\n")
